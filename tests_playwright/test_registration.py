...
def test_registration_success(reg_page):
    ...
    with allure.step(f"Заполнить форму данными пользователя: {user.email}"):
        ...
        reg_page.set_policy_checkbox(True)

    with allure.step("Отправить форму регистрации"):
        reg_page.submit_registration()

    with allure.step("Проверить сообщения об успешной регистрации и логине"):
        reg_page.assert_confirmation_text("Registered")
        reg_page.assert_confirmation_text_1("You are logged in success")
        reg_page.close_window()


@allure.epic("UI Testing (Playwright)")
@allure.feature("Registration Page")
@allure.story("Negative Registration - Fields Validation")
@allure.title("Валидация обязательных полей и форматов при регистрации")
...
def test_registration_negative_fields(reg_page, field_name, invalid_value, expected_error):
    ...
    with allure.step(f"Заполнить форму с невалидным значением в поле '{field_name}': '{invalid_value}'"):
        reg_page.fill_registration_form(user)
        reg_page.set_policy_checkbox(True)
        reg_page.remove_focus()

    with allure.step(f"Проверить текст ошибки '{expected_error}' и блокировку кнопки Submit"):
        reg_page.assert_error_message(expected_error)
        reg_page.assert_submit_button_disabled(disabled=True)
...
@allure.severity(allure.severity_level.NORMAL)
def test_registration_without_check_box(reg_page):
    user = UserGenerator.get_random_user()

    with allure.step("Открыть форму регистрации и заполнить валидные данные"):
        reg_page.open_registration_form()
        reg_page.fill_registration_form(user)
        reg_page.set_policy_checkbox(True)

    with allure.step("Снять чекбокс пользовательского соглашения (False)"):
        reg_page.set_policy_checkbox(False)
        reg_page.remove_focus()

    with allure.step("Проверить появление ошибки о необходимости принять условия и блокировку кнопки"):
        reg_page.assert_error_message("You must accept the terms")
        reg_page.assert_submit_button_disabled(disabled=True)
...
@allure.title("Проверка обработки пробелов (whitespace) в полях формы регистрации")
@allure.severity(allure.severity_level.NORMAL)
@pytest.mark.parametrize(
    ...
    ids=[
        ...
        "spaces_in_email",
        "spaces_in_password",
        "leading_trailing_spaces_in_names",
        "leading_trailing_spaces_in_email"
    ]
)
def test_registration_whitespace_handling(
        reg_page: RegistrationPlaywrightPage,
        name: str,
        last_name: str,
        email: str,
        password: str,
    ...
    with allure.step("Ввести тестовые данные с пробелами и убрать фокус"):
        reg_page.fill_name(name)
    ...
    if test_type == "error":
        with allure.step(f"Ожидаем ошибку валидации: '{expected}'"):
            reg_page.assert_error_message(expected)
    elif test_type == "success":
        with allure.step("Отправить форму"):
            reg_page.submit_registration()
        with allure.step(f"Ожидаем подтверждение: '{expected}'"):
            reg_page.assert_confirmation_text(expected)